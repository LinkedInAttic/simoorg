...
Structure of the config directory is discussed details later on in this document. Once Moirai becomes aware of itself, it spawns individual observers called Atropos for each service defined in the configs. Each Atropos instance acts as an observer for the service it is responsible for and dispatches actions following a plan generated by Scheduler. 

Here each Atropos can communicate specific information to Moirai, with the help of two multiprocessing queues: a service information queue and an event queue. The service information queue captures information about Atropos like:  service name, topology of that service and the plan which the current Atropos will be following. Event queue captures the status of each failure and revert actions performed by Atropos. High level design is reflected in the following diagram:
...
Each Atropos instance has its own instance of a Scheduler which is in charge of generating failure plan and keeping track of time. There can be different schedulers configured, however Simoorg is currently shipped with non-deterministic scheduler at this time.

Apart from the Scheduler, each Atropos instance has its own instance of a Handler,  Logger and Journal. High level diagram reflecting Atropos and its components is as follows:
![Atropos Components](/docs/images/atr1.png)

###Scheduler

Scheduler generates a failure plan and keeps track of time. Currently Simoorg ships only with a Non-deterministic scheduler out of the box. Non-deterministic scheduler randomly generates dispatch times and associates them with random failures. We refer to this sequence of timestamp and failures internally as a Plan. Once generated, the Plan is passed to Atropos.


###Handler

Each failure definition should have a handler associated with it. Handler is referred by its name within a failure definition and is responsible for inducing or reverting failures. The table below lists supported handlers and handlers planned to be available in future:
...
Each Observer has a separate Journal instance. Journal is responsible for:

* Keeping track of the internal state of Atropos such as: current impact and impact limits
* Persisting current state of Atropos to support session resumption
* Resuming state after a crash

###Logger

Each Atropos has a separate Logger instance. Logger is used to log and store arbitrary messages spit out at various points of the Plan execution.

###HealthCheck

Healthcheck is an optional component that allows you to control the damage inflicted against your service. If enabled, Atropos kicks off the healthcheck logic defined in the Fate Book before inducing a failure. Healthcheck component need to return success to let the failure run. Otherwise Scheduler skips the current failure cycle. This ensures that we are not aggravating any existing issues and let the cluster to fire self-healing routines and recover. If a healthcheck is not defined, failures will be induced as scheduled assuming the cluster was able to recover.

Default implementation of the Healthcheck component allows to plug in any executable with a logic. A sample healthcheck would like like this:
```
#!/bin/bash

if [ A_CHECK_SUCCESSFUL ]
then
   exit 0
else
   exit 1
...
Another example of topology is Kafka topology. It is a custom Topology component that was developed to support dynamic Kafka node elections. Topology plugin for kafka returns the list of different types of brokers on which failures can be induced. The plugin selects random brokers based on the kafka host resolution specification in the config file. Each host resolution specification should belong to one of the following types.
* CONTROLLER - Where the node is the controller of the kafka cluster
* RANDOM_BROKER - Where node is a broker for a specific topic and a specific partition (if you skip the topic or partition it randomly selects a topic or partition)
* RANDOM_LEADER - Where node is a leader for a random topic and a random partition
* LEADER - Where node is a leader for a specific topic and a specific partition (if you skip the partition it randomly selects a partition)
...
* GET /list - Gives the list of services against which simoorg is running (i.e list of service-name provided in the fate books)
* GET <service-name>/servers - Gives the list of components (for example servers) that constitutes a service (Output of topology plugin for that service)
